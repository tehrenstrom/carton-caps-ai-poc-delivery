    ...
    if asyncio.iscoroutinefunction(func):
            ...
            try:
                ...
                if session:
                    try:
                        ...
                        logger.warning(f"Rolled back session due to unhandled exception in {func.__name__}.")
                    except Exception as rollback_err:
                        logger.error(f"Failed to rollback session in {func.__name__} after error: {rollback_err}")
        ...
        def sync_wrapper(*args, **kwargs):
            ...
            try:
                ...
                if session:
                    try:
                        logger.warning(f"Session present during exception in sync endpoint {func.__name__}. Manual rollback might be needed if transactions occurred.")
                    except Exception as rollback_err:
                        logger.error(f"Error during session check/rollback attempt in {func.__name__} after error: {rollback_err}")
                raise HTTPException(status_code=500, detail="Internal Server Error")
        return sync_wrapper
...
async def get_products(limit: int = 100, session: AsyncSession = Depends(get_async_session)):
    """Get all products with optional limi"""
