...
-   **API/Integration Tests (`tests/test_api.py`, `tests/test_chat_flow.py`):**
    -   These tests use FastAPI's `TestClient` (via `httpx.AsyncClient`) to make live requests to the API endpoints (`/chat`, `/products`, `/faqs`, `/referral-rules`, etc.).
    -   They verify the full request/response cycle, including database interactions (CRUD operations) when a `DATABASE_URL` is configured in the environment, and conversation flow logic.
    -   Includes tests for both happy paths and error handling (e.g., invalid user ID, conversation context).
    -   One test (`test_chat_endpoint_real_db_and_llm` in `test_api.py`) specifically attempts a chat interaction that may involve a call to the actual LLM service if configured.
    -   **Async Testing:** Includes a specialized test (`test_chat_endpoint_is_async` in `test_api.py`) that verifies the asynchronous behavior of the API, ensuring concurrent requests are handled efficiently without blocking.
...
Use the setup.sh setup script to quickly set up your development environment:
...
Option 1: Automatic setup with sample data:
```bash
